...
P1s = map([1, 0]) do h
    ...
    map(enumerate(Lss)) do (i,Ls)
        ...
        A = vcat(Ab, Af)
        ps = map(mss) do ms
            M = MainlandIslandModel(HapDipDeme(N=N, k=k, u=u, A=A), ms*sf, ones(L+1))
        ...
        P1 = plot!(mss, ps, color=[i i], alpha=[0.3 1], lw=[2 1.5],
                  ...
                  title="\$N_e = $Ne, h=$h, s_f=$sf\$")
    end
    ps2 = map(mss) do ms
        M = MainlandIslandModel(HapDipDeme(N=N, k=k, u=u, A=Af), ms*sf, ones(1))
...
P2s = map([1, 0]) do h
    Af = Architecture(DipLocus(-h*sf, -sf), 1)
    map(zip([4,6], [:solid, :solid])) do (sm, ls)
        PP = plot(ylim=(-15,2))
        m = sf/sm
        M = MainlandIslandModel(HapDipDeme(N=N, k=k, u=u, A=Af), m, ones(1))
        P,_= fixedpointit(M, ones(1));
        p  = P[end,:,1]
        pq = P[end,:,2]
        ...
        map(enumerate(Lss)) do (i,Ls)
            ...
            Ab = Architecture(DipLocus(-0.5s, -s ), L)
            A = vcat(Ab, Af)
            M = MainlandIslandModel(HapDipDeme(N=N, k=k, u=u, A=A), m, ones(L+1))
            P,_ = fixedpointit(M, ones(2))
            P[end,:,1]
            p  = P[end,:,1]
            pq = P[end,:,2]
...
plot(P11, P12, layout=(2,1), size=(630,340), legendfont=7, titlefont=9)
