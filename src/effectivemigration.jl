...
# XXX don't think these are right with diploid migration
function eqgff(M::MainlandIsland)
    ds = equilibrium(M)
    eqgff(M, ds)
end

# HACKy assumes equal mutation rates...
function eqgff(M::MainlandIsland, ds::Vector{Wright{T}}) where T
    @unpack A = M.deme 
    map(i->(ds[i].B - ds[i].A)/(2M.deme.Ne), 1:length(ds))
end
